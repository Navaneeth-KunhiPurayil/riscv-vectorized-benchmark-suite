	...
	for(l=0; l<dim.number_boxes; l=l+1){
		...
		for (k=0; k<(1+box[l].nn); k++)
		{
			...
			for (i=0; i<NUMBER_PAR_PER_BOX; i=i+1){
			    ...
			    // Avoid compiler optimizing it into a vector store operation
				asm volatile ("vfmv.f.s %0, %1" : "=f"(fA[i].v) : "vr"(xfA_1_v));
				asm volatile ("vfmv.f.s %0, %1" : "=f"(fA[i].x) : "vr"(xfA_1_x));
				asm volatile ("vfmv.f.s %0, %1" : "=f"(fA[i].y) : "vr"(xfA_1_y));
				asm volatile ("vfmv.f.s %0, %1" : "=f"(fA[i].z) : "vr"(xfA_1_z));
			} // for i
